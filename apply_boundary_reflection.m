...
function [xPos, yPos, direction_angle] = apply_boundary_reflection(xPos, yPos, boundaryX, boundaryY, sensing_radius, direction_angle)
    ...
    if min_dist < sensing_radius
        % reflect when robot is within 1/3 of sensing radius
        if min_dist < 1/2 * sensing_radius
            ...
            xPos = closest_boundary_x + sensing_radius/2 * normal_x;
            yPos = closest_boundary_y + sensing_radius/2 * normal_y;
        end
    end
end